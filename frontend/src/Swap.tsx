...
function TokenForm(props : any) {
  const location = (props.tokenForSale == 0 && props.name == "amountLink") || (props.tokenForSale == 1 && props.name == "amountUsdc") ?
   10 : 20;
  const img = (props.name == "amountLink" ? <img src={link} className="link" /> : <img src={usdc} className="usdc" />);
    return (  <label><View style={[{ top : location, left : 60, width : 30}]} >
...
function MaxSlippage(props : any) {
    return (<label>
    Maximum slippage in %:
    <input type="number" name="maxSlippage" value={props.maxSlippage} onChange={props.handleChange} />
    </label>)
}
...
class Swap extends React.Component<any, any>{
    ...
    changeForms = async () => {
        //const sellForm  = this.state._buyForm;
        //const buyForm  = this.state._sellForm;
        //const tokenForSale = (this.state.tokenForSale + 1) % 2;
    ...
    swap = async() =>  {
      ...
      if (amountSoldToken <= 0)
        ...
        const amountSoldTokenStr = getBigNumberWithDecimals(amountSoldToken * 1.1, numberDecimalsSoldToken).toString();
        ...
        //await approve_and_wait(amountSoldTokenStr, simpleSwapInfo["contractAddress"], soldTokenContractAdress, soldTokenAbi);
        ...
        console.log("before calling contract");
    ...
    render() {
      ...
      <View >
      <MaxSlippage amount={this.state.maxSlippage} handleChange={async(event : any) => {this.handleChange(event);
                                                           this.changeMinAmountReceived()}}/>
      ...
      < this.state._sellForm  name={this.state.tokenForSale == 0 ? "amountLink" : "amountUsdc" } amount={this.state.tokenForSale == 0 ? 
                  this.state.amountLink : this.state.amountUsdc} tokenForSale = {this.state.tokenForSale} handleChange={async(event : any) =>
                                          {await this.handleChange(event); await this.changeInput(event);
                                            await this.changeMinAmountReceived() }}/>
    
       </div>
       <div className="Switch-btn">
      
      <Button title="Switch"  color="#02882f" onPress={this.changeForms}/>
